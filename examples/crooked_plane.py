...
class DefaultCrookedPlane(Shape):
    ...
    def intersect_wing1(self, ray):
        return self.intersect_on_plane(ray, -1)

    def intersect_wing2(self, ray):
        return self.intersect_on_plane(ray, 1)

    def intersect_on_plane(self, ray, plus_or_minus):
        if ray.direction.y == plus_or_minus * ray.direction.z:  # parallel to the wing
            return None

        t = plus_or_minus * (ray.origin.y + ray.origin.z)/(ray.direction.y + ray.direction.z)
        intersection_point = ray.origin + t * ray.direction
        if not in_bounding_box(intersection_point):  # Clip it
            return None
        if plus_or_minus * intersection_point.x <= 0:
            return Intersection(t, self)
